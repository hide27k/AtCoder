    ...
    static void solve() {
        ...
        for (int i = 1; i <= N; i++) {
            for (int j = 1; j <= K; j++) {
                for (int k = 0; k <= W; k++) {
                    if (k >= screen[i - 1][0]) {
                        dp[i][j][k] = Math.max(Math.max(dp[i - 1][j][k], dp[i - 1][j - 1][k]),
                                               dp[i - 1][j - 1][k - screen[i - 1][0]] + screen[i - 1][1]);
                    } else {
                        dp[i][j][k] = Math.max(dp[i - 1][j - 1][k], dp[i - 1][j][k]);
                    }
                }
            }
        }
